...
Since May 2020 <span style="background-color: #dfc77f">**Mirai Solutions**</span> is <mark>showing</mark> a dashboard on our [gallery]({{ site.baseurl }}/gallery/) that contains a global view of the [COVID-19 Pandemic]({{ site.baseurl }}/gallery/covid19/), with a further split by continent and country. We use publicly available data from the [COVID-19 Data Hub](https://covid19datahub.io/), a great open source project providing a unified data set put together from numerous official local sources from all over the world.
...
Here we provide a live update to these articles embedded in [shinyapps.io](https://www.shinyapps.io/) that will always show the latest data from BAG.     
...
As of Today, (**`r Sys.Date()`**), the 4 last weeks considered are: **`r weeks4`**, i.e. in the interval from **`r weeks_from_to[1]`** to **`r weeks_from_to[2]`**.
...
The current situation in the last 4 weeks as of **`r weeks_from_to[2]`**: how the infections, hospitalizations and deaths occurred across the age classes in absolute terms is being shown below. Overall Switzerland has registered <span style="background-color: #dfc77f">**`r .extract_value(datacases, "Infections")`**</span> infections, <span style="background-color: #dfc77f">**`r .extract_value(datacases, "Hospitalizations")`**</span> hospitalizations and <span style="background-color: #dfc77f">**`r .extract_value(datacases, "Deaths")`**</span> deaths.


```{r ageclasses-pandemic-month, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
```{r ageclasses-pandemic-month-100k, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
The current vaccination status per age group as of **`r weeks_from_to[2]`**, the "Fully Vaccinated" population is split according to the occurred injection of the **Booster** dose.

```{r ageclasses-vacc-perc, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
Pop <- DATA.AG1M %>%
    filter((!AgeClass %in% c("unknown"))) %>%
    select(-Week) %>%  
    group_by(AgeClass) %>%
...
**`r datavacall %>% filter(Status %in% names(vac_levels())[vac_levels() == "fully_vaccinated_no_booster"]) %>% .[,"Value"]`** are fully vaccinated (2 doses), **`r datavacall %>% filter(Status %in% names(vac_levels())[vac_levels() == "partially_vaccinated"]) %>% .[,"Value"]`** have received at least one dose, while **`r datavacall %>% filter(Status %in% names(vac_levels())[vac_levels() == "fully_vaccinated_first_booster"]) %>% .[,"Value"]`** of Swiss residents have received the booster dose.

This page mainly focuses on the comparison between the **Vaccinated** and **Unvaccinated**. It is worth first highlighting the differences between the 2 populations that would bias such comparison.
...
```{r ageclasses-tot-confirmed, warning=FALSE}
...
TotConfirmed %>%
  ...
  tidyHtmlTable(header = Case ,
                #cgroup = c(Case),
                rnames = AgeClass,
                caption = paste("Table 0: Confirmed Infections per Age Class.", weeks_from_to[2])) #%>%
  # kableExtra::save_kable("table0.png", zoom = 1.45,expand = 10, vwidth = 666)
...
* Leading to underestimation of vaccination effect:  
  ...
  + Hospitalized "with" Covid, i.e. recorded positive upon hospitalization, are counted as Covid patients.   
* Leading to overestimation of vaccines effect:   
  + The "Vaccinated" may lead a healthier life and hence be less at risk (?).   
  + A portion of "Unvaccinated" is being very incautious and exposes itself to higher risk, e.g. participating in demonstrations, denying Covid, taking no protections etc. (?).   
...
```{r ageclasses-vacc-unvac, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
```{r ageclasses-vacc-unvac-tab-pop-infect, warning=FALSE}
DataTab = DataPlot %>%
  rename(Status = vaccination_status) %>%
  rename(value = Value)
...
DataTab %>%
  ...
  tidyHtmlTable(header = Status ,
                cgroup = c(Case),
                rnames = AgeClass,
                caption = paste("Table 1: absolute entries per age and vaccination status.", periodLabel)) #%>%
  #kableExtra::save_kable("table1.png", zoom = 1.45,expand = 10, vwidth = 666)
```
<!-- ![](table1.png) -->
<br>
...
DataTabScale <- rescale_unkwnown(DataTab)
```
```{r ageclasses-vacc-unvac-tab4-tab, warning=FALSE}
DataTabScale %>%
  mutate(value = formatC(round(value), format = "f", big.mark = "'", digits = 0)) %>%
  ...
  tidyHtmlTable(header = Status ,
                cgroup = Case,
                rnames = AgeClass,
                caption = paste("Table 4: entries per age and vaccination status. Reallocation of Unknown vaccination status.", periodLabel)) #%>%
  #kableExtra::save_kable("table4.png", zoom = 1.45,expand = 10, vwidth = 666)
...
```{r ageclasses-vacc-unvac-scale100k-plot, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
DataPlot100kScale2 = DataPlot100kScale %>%
  #filter(AgeClass != "All" & Case != "Population")
...
```{r ageclasses-vacc-unvac-scale100k-graph, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
```
```{r ageclasses-vacc-unvac-scale100k-tab, warning=FALSE}
DataPlot100kScaleRatios %>%
  ...
  tidyHtmlTable(header = Status ,
                cgroup = c(Case,Variable),
                rnames = AgeClass,
                caption = paste("Table 5: entries over 100'000 people per age and vaccination status. Reallocation of Unknown vaccination status.", periodLabel),
                skip_removal_warning = TRUE) #%>%
#   kableExtra::save_kable("table5.png", zoom = 1.45,expand = 10, vwidth = 666)

```
<!-- ![](table5.png) -->
The measures in the table above for the Age Class "All" are age adjusted.
...
```{r ageclasses-cases-scenarios-100k, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
# DataPlot100kpTab7 = DataPlot100kp %>%
#   filter(AgeClass != "All")
...
```{r ageclasses-cases-scenarios, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
```{r ageclasses-cases-scenarios-tab,warning=FALSE, echo = FALSE, include = TRUE}
# TotDataPlotp = DataPlotp %>%
#   filter(AgeClass != "All") %>%
#   group_by(AsOfDate,Status,Case) %>%
#   summarize(Value = sum(Value, na.rm = TRUE)) %>%
#   mutate(AgeClass = "Total")

# DataTabp <- DataPlotp %>% select(-pop, AsOfDate) %>%
#   filter(AgeClass != "All") %>%
#   bind_rows(TotDataPlotp) %>%
#   rename(value = Value) 
# 
# TotDataTabp <- DataTabp %>% group_by(Status,Case) %>% summarize(tot = sum(value, na.rm = TRUE))
...
week_to_consider <- tail(unique(DATA.AGG2$Week), 30)
...
for (week in week_to_consider) {
  ...
  DataMonthScale <- rescale_unkwnown(DataMonth) %>%
    rename(Value = value)
...
first.week.booster = DATA.AGG2 %>% 
  filter(AgeClass == "All", vaccination_status == "Fully vac. Booster")  %>% select(Week, pop) %>%
  left_join(DATA.AGG2 %>% filter(AgeClass == "All") %>% group_by(Week) %>% 
  summarise(sumpop = sum(pop)), by = "Week") %>%
  mutate(VaxPop = pop/sumpop) %>% filter(VaxPop > threshold.vax) %>% .[1, "Week"]
...
<!-- # ```{r ageclasses-pandemic-hosp-timeline, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'} -->
<!-- #  -->
<!-- # LinePlotCovid(DataRoll4W100k %>% filter(Case == "Hospitalizations" & AsOfDate %in% last_timeline_weeks),  -->
<!-- #               FACET = "AgeClass", g_palette = c(rev(VaxCol),NoVaxCol),  -->
<!-- #               title = "Time-line of Hospitalizations per Age class")  -->
<!-- # ``` -->
<!-- <div class = "moveup"> -->
<!-- <div class = "extendediframe"> -->
<!-- ```{r first-app-plot-reactive-app_read, echo=TRUE, eval=TRUE, include=TRUE} -->
<!-- knitr::read_chunk("shinyapps/TimeLinePlot.R") -->
<!-- ``` -->
<!-- </div> -->
<!-- </div> -->
<!-- # ```{r ageclasses-pandemic-death-timeline, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'} -->
<!-- #  -->
<!-- # LinePlotCovid(DataRoll4W100k %>% filter(Case == "Deaths" & AsOfDate %in% last_timeline_weeks), FACET = "AgeClass", g_palette = c(rev(VaxCol),NoVaxCol),  -->
<!-- #               title = "Time-line of Deaths per Age class")  -->
<!-- # ``` -->

<!-- The curves related to the whole population ("All") look closer because the "Unvaccinated" population is much younger (they are not age adjusted). -->
<!-- Due to the larger values in the "Unvaccinated" population the curves among the "Fully Vaccinated" look pretty flat and it is worth having a better look at them. -->

<!-- ```{r ageclasses-pandemic-hosp-timeline-fullyvac, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'} -->

<!-- LinePlotCovid(DataRoll4W100k %>% filter(Case == "Hospitalizations" & AsOfDate %in% last_timeline_weeks & Status %in% grep("Fully", names(vac_levels()), value = TRUE)), -->
<!--               FACET = "AgeClass", g_palette = c(rev(VaxCol)), -->
<!--               title = "Time-line of Hospitalizations per Age class among the Fully Vaccinated") -->
<!-- ``` -->
<!-- ```{r ageclasses-pandemic-deaths-timeline-fullyvac, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'} -->

<!-- LinePlotCovid(DataRoll4W100k %>% filter(Case == "Deaths" & AsOfDate %in% last_timeline_weeks & Status %in% grep("Fully", names(vac_levels()), value = TRUE)), -->
<!--               FACET = "AgeClass", g_palette = c(rev(VaxCol)), -->
<!--               title = "Time-line of Deaths per Age class among the Fully Vaccinated") -->
<!-- ``` -->

<!-- The ratio between the incidence of "Unvaccinated" vs "Fully Vaccinated" over timee gives us a measure of how higher is the risk for the "Unvaccinated" is. -->
...
# assign("DataRoll4W100k", DataRoll4W100k, pos = 1)
# assign("last_timeline_weeks", last_timeline_weeks, pos = 1)

# assign("DataRoll4W100k", DataRoll4W100k, envir = .GlobalEnv)
# assign("last_timeline_weeks", last_timeline_weeks, envir = .GlobalEnv)

# saveRDS(list(data = DataRoll4W100k, weeks = last_timeline_weeks), "chunk1.rds")
...
```
<!-- # ```{r ageclasses-pandemic-hosp-timeline_ratio-graph, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'} -->
```{r ageclasses-pandemic-hosp-deaths-timeline_100k-graph, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-responsive"'}
...
data <- DataRoll4W100k %>% filter(AsOfDate %in% last_timeline_weeks)
#SERVER
renderPlotly({
...
```{r ageclasses-pandemic-hosp-deaths-timeline_ratio, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-centered img-responsive"'}
...
```
```{r ageclasses-pandemic-hosp-deaths-timeline_ratio-graph, warning=FALSE, fig.width=6.667, fig.asp=0.6, dpi=150, out.extra='class="img-responsive"'}
...
data <- DataRoll4W100kratio %>% filter(AsOfDate %in% last_timeline_weeks)
#SERVER
renderPlotly({
      if (is.null(input$case))
        return(plot.new())
